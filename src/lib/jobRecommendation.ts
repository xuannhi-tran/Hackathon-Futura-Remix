...
const CONFIG = {
  ...
  partial: {
    snippetRole: 0.75,
    unknownLocation: 0.5,
    regionalLocation: 0.9,
    unknownExperience: 0.5,
  },
  penalties: {
    experience: 15,
    seniorBeginner: 30,
...
export function recommendJob(
  ...
  const text = normalise(
    `${job.title} ${job.description ?? ""} ${job.contractTime ?? ""} ${
      job.contractType ?? ""
    }`
  );
  ...
  const titleMatches = target.filter((word) =>
    titleWords.includes(word)
  ).length;
  const allMatches = target.filter((word) => allWords.includes(word)).length;
  const role = target.length
    ? Math.round(
        weights.role *
          Math.max(
            titleMatches / target.length,
            (partial.snippetRole * allMatches) / target.length
          )
      )
    : 0;
  reasons.push(
    `Role fit +${role}/${weights.role}: ${
      role
        ? `keyword overlap with ${profile.targetField}`
        : "no target-field keyword match in the available snippet"
    }.`
  );
  ...
  const location = isAnywhere
    ? weights.location * partial.unknownLocation
    : matchesState(locationText, preferred)
    ? weights.location
    : 0;

  reasons.push(
    `Location fit +${location}/${weights.location}: ${
      isAnywhere
        ? "no state preference set"
        : location > 0
        ? `matches ${preferred}`
        : "preferred state not matched"
    }.`
  );

  // Seniority is read from the title so 'work with senior staff' is not a senior role.
  const senior = /\b(senior|sr|lead|principal|staff|head|director)\b/.test(
    title
  );
  const junior = JUNIOR_TITLE_PATTERN.test(title);
  const yearMatches = [
    ...text.matchAll(
      /\b(\d+)(?:\s+(?:to\s+)?\d+)?\s*\+?\s*years?\s+(?:of\s+)?(?:relevant\s+|professional\s+|commercial\s+)?experience\b/g
    ),
  ];
  const requestedYears = yearMatches.length
  ...
  if (requestedYears !== undefined) {
    experience =
      profile.yearsExperience >= requestedYears ? weights.experience : 0;
    ...
    if (profile.yearsExperience < requestedYears)
      deduct(
        ...
        "Advertised experience exceeds your profile."
      );
  } else if (junior) {
    experience =
      profile.yearsExperience <= thresholds.juniorYears
        ? weights.experience
        : weights.experience * partial.unknownExperience;
  ...
  if (senior && profile.yearsExperience < thresholds.seniorYears) {
    experience = 0;
    deduct(
      profile.yearsExperience < thresholds.beginnerYears
        ? deductions.seniorBeginner
        : deductions.seniorStretch,
      "Senior, lead, principal or staff-level title is a stretch for your experience."
    );
  }
  reasons.push(
    `Experience fit +${experience}/${weights.experience}: ${experienceReason}.`
  );
  ...
  const hourMatches = [
    ...text.matchAll(
      /\b(\d+(?:\.\d+)?)(?:\s+(?:to\s+)?(\d+(?:\.\d+)?))?\s*(?:hours?|hrs?)\s*(?:per|a|each)?\s*(week|weekly|fortnight|fortnightly)\b/g
    ),
  ];
  const highHours = hourMatches.some(
    (match) =>
      Number(match[2] ?? match[1]) >
      (match[3].startsWith("fortnight")
        ? thresholds.fortnightlyHours
        : thresholds.weeklyHours)
  );
  const workPattern = studyTerm && (/\bfull time\b/.test(text) || highHours);
  if (workPattern) {
    deduct(
      deductions.workPattern,
      "Full-time or high-hours wording may conflict with your study-term work pattern."
    );
  }
  ...
  const restrictions = [
    {
      pattern:
        /\b(?:australian citizens?(?:hip)?|citizens?(?:hip)? only|permanent residents?(?: only)?|permanent residency|pr only)\b/,
      summary: "citizenship or permanent residency",
      label: "Citizenship or permanent-residency wording",
    },
    {
      pattern: /\b(?:security clearance|nv1|nv2|baseline clearance)\b/,
      summary: "security clearance",
      label: "Security-clearance wording",
    },
    {
      pattern:
        /\b(?:(?:full|unrestricted)(?: australian)? (?:work|working) rights|work(?:ing)? without restriction)\b/,
      summary: "unrestricted work rights",
      label: "Full or unrestricted work-right wording",
      studyTermOnly: true,
    },
  ];
  ...
  const detected = restrictions.filter(
    ({ pattern, studyTermOnly }) =>
  ...
  if (detected.length)
    deduct(
      ...
      `${detected
        .map(({ label }) => label)
        .join("; ")} appears in the snippet; review the full ad.`
    );

  const durations = [
    ...text.matchAll(
      /\b(\d+)\s*(month|year)s?\s+(?:fixed term\s+)?contract\b/g
    ),
  ];
  const durationMismatch =
    ...
    durations.some(
      (match) =>
        Number(match[1]) * (match[2] === "year" ? 12 : 1) >
        profile.monthsRemaining!
    );
  if (durationMismatch) {
    deduct(
      deductions.duration,
      `Duration/profile mismatch: advertised contract duration exceeds your ${profile.monthsRemaining} months remaining. This is a soft ranking adjustment, not a determination of legal eligibility.`
    );
  }
  reasons.push(
    `Profile fit +${weights.profile}/${weights.profile} before deductions: only explicit snippet work-pattern signals are assessed; missing wording does not establish eligibility.`
  );
  ...
  if (detected.length || workPattern) {
    signals.push({
      ...
      text: detected.length
        ? `Summary mentions ${detected
            .map(({ summary }) => summary)
            .join(", ")}; check the full ad.${
            workPattern
              ? " Work hours may also conflict with your study-term pattern."
              : ""
          }`
  ...
  signals.push({
    id: "role",
    tone: strongRole ? "positive" : role > 0 ? "neutral" : "caution",
    text: strongRole
      ? `Strong match for ${profile.targetField}`
      : role > 0
      ? `Some overlap with ${profile.targetField}`
      : `Limited evidence of a match for ${profile.targetField}`,
  });
  signals.push({
    id: "location",
    tone: isAnywhere ? "neutral" : location > 0 ? "positive" : "caution",
    text: isAnywhere
      ? "No preferred state set"
      : location > 0
      ? `Matches your preferred state: ${preferred}`
      : "Preferred state not matched in the summary",
  });
  const experienceMismatch =
    (requestedYears !== undefined &&
      profile.yearsExperience < requestedYears) ||
    (senior && profile.yearsExperience < thresholds.seniorYears);
  const experienceUnknown = requestedYears === undefined && !junior && !senior;
  signals.push({
    id: "experience",
    tone: experienceMismatch
      ? "caution"
      : experienceUnknown
      ? "neutral"
      : experience === weights.experience
      ? "positive"
      : "neutral",
    text: experienceMismatch
      ? "Role may require more experience than your profile lists"
      : experienceUnknown
      ? "Experience requirement not specified"
      : experience === weights.experience
      ? "Experience level appears to match your profile"
  ...
  if (durationMismatch) {
    signals.push({
      id: "duration",
      tone: "caution",
      text: "Contract duration may extend beyond your current visa timeframe",
    });
  }
  return {
    score: Math.max(
      ...
      Math.min(
        CONFIG.score.max,
        role + location + experience + weights.profile - penalty
      )
    ),
    signals,
    reasons,
